...
Integer variable of SC type ({\tt sct\_uint}, {\tt sct\_int}, {\tt sc\_uint}, {\tt sc\_int}, {\tt sc\_biguint}, {\tt sc\_bigint}) can be initialized from string literal or string variable. The string should contain correct integer value in any radix supported. For {\tt sct\_uint}, {\tt sct\_int}, {\tt sc\_biguint} and {\tt sc\_bigint} initialization value could be more than 64 bits. String variable should be initialized at elaboration phase (in the module constructor).
...
Normally ICSC does not allow to have latch in SystemC source, but there are some cases where latch is required. There is ICSC assertion {\tt sct\_assert\_latch} which intended to specify latch in method process, see~\ref{section:latch_assert}. It suppresses ICSC error message for latch variable, signal or port. 
...
\subsection{Logging and others}\label{section:logging}

SystemC logging have multiple options including standard output stream {\tt std::cout}, {\tt printf} function and {\tt sc\_trace} function.

These debugging functions are allowed in the code for synthesis. {\tt std::cout} together with {\tt operator <<} are ignored as well as {\tt sc\_trace} function. 
For {\tt printf} function an equivalent {\tt \$display} macro is generated in SV code. {\tt printf} should have format string parameter and one or more variables to print. For SystemC type variables conversion to C++ types should be applied.
