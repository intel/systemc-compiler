#!/usr/bin/python3
#
# Copyright (c) [2022-2025] [Leonid Azarenkov, Intel]
#
import gdb
...
class ScBigInt_ScBigUInt:
    ...
    def to_string(self):
        ...
        sgnbidx  = (nbits - 1) % BITS_PER_DIGIT
        sign_bit = int(self.val["digit"][ndigits-1]) >> sgnbidx

        digmask  = (2 << sgnbidx) - 1
        intval = int(self.val["digit"][ndigits - 1]) & digmask
        for i in reversed(range(0, ndigits - 1)):
            digit = int(self.val["digit"][i])
            intval = (intval << BITS_PER_DIGIT) + digit

        if sign_bit != 0: #convert to signed negative value
            valmask = (1 << nbits) - 1
            intval = -((intval ^ valmask) + 1)
...
def build_pretty_printer():
    sysc30x_pp = gdb.printing.RegexpCollectionPrettyPrinter("SysC-30X")

    sysc30x_pp.add_printer("sc_bit", "^sc_dt::sc_bit$", ScBit)
    sysc30x_pp.add_printer("sc_bv_base", "^sc_dt::sc_bv<(.*)>$", ScBv)
    sysc30x_pp.add_printer("sc_logic", "^sc_dt::sc_logic$", ScLogic)
    sysc30x_pp.add_printer("sc_lv", "^sc_dt::sc_lv<(.*)>$", ScLV)
    sysc30x_pp.add_printer("sc_int", "^sc_dt::sc_int<(.*)>$", ScInt_ScUInt)
    sysc30x_pp.add_printer("sc_uint", "^sc_dt::sc_uint<(.*)>$", ScInt_ScUInt)
    sysc30x_pp.add_printer("sc_bigint", "^sc_dt::sc_bigint<(.*)>$", ScBigInt_ScBigUInt)
    sysc30x_pp.add_printer("sc_biguint", "^sc_dt::sc_biguint<(.*)>$", ScBigInt_ScBigUInt)
    sysc30x_pp.add_printer("sc_signed", "^sc_dt::sc_signed$", ScBigInt_ScBigUInt)
    sysc30x_pp.add_printer("sc_unsigned", "^sc_dt::sc_unsigned$", ScBigInt_ScBigUInt)
    sysc30x_pp.add_printer("sc_vector", "^sc_core::sc_vector<(.*)>$", ScVector)
    sysc30x_pp.add_printer("sc_signal", "^sc_core::sc_signal<(.*)>$", ScSignal)
    sysc30x_pp.add_printer("sc_in", "^sc_core::sc_in<(.*)>$", ScSigPort)
    sysc30x_pp.add_printer("sc_out", "^sc_core::sc_out<(.*)>$", ScSigPort)
    sysc30x_pp.add_printer("sc_inout", "^sc_core::sc_inout<(.*)>$", ScSigPort)

    return sysc30x_pp


def register_sysc30x_printers(val):
