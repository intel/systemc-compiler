...
// Next-state combinational logic
always_comb begin : resp_sync_thread_comb     // sct_initiator.h:212:5
...
function void runProc_func;
    ...
    case (runProc_PROC_STATE)
        0: begin
            ...
            if (!run_core_req && !run_reg_full)
            begin
                runProc_PROC_STATE_next = 0; return;    // sct_target.h:182:52;
            end
            run_get_req_next = !run_get_req;
            if (run_reg_full)
            begin
...
function void respProc_func;
    ...
    case (respProc_PROC_STATE)
        0: begin
            ...
            if (!fifo_out_valid)
            begin
                respProc_PROC_STATE_next = 0; return;    // sct_fifo.h:153:32;
            end
            fifo_get_req_next = !fifo_get_req;
            TMP_0 = fifo_data_out;
            // Call b_get() end
            ...
            if (!resp_core_ready)
            begin
                respProc_PROC_STATE_next = 2; return;    // sct_initiator.h:172:37;
            end
            resp_sync_req_next = !resp_sync_req;
