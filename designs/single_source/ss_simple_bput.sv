...
function void runProc_func;
    ...
    case (runProc_PROC_STATE)
        0: begin
            ...
            if (!run_core_req && !run_reg_full)
            begin
                runProc_PROC_STATE_next = 0; return;    // sct_target.h:188:52;
            end
            run_get_req_next = !run_get_req;
            if (run_reg_full)
            begin
            ...
            if (!fifo_ready_push)
            begin
                runProc_PROC_STATE_next = 2; return;    // sct_fifo.h:197:33;
            end
            fifo_put_req_next = !fifo_put_req;
            // Call b_put() end
...
function void respProc_func;
    ...
    case (respProc_PROC_STATE)
        0: begin
            ...
            if (!fifo_out_valid)
            begin
                respProc_PROC_STATE_next = 0; return;    // sct_fifo.h:159:32;
            end
            fifo_get_req_next = !fifo_get_req;
            TMP_0 = fifo_data_out;
            // Call b_get() end
