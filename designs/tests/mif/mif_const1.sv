...
begin : aminst_mifProc_sct_i 
    ...
    function void aminst_mifProc_func;
        ...
        case (mifProc_PROC_STATE[sct_i])
            0: begin
                LB_next[sct_i] = 6'd58;
                LC = 6'd59;
                j = 32'(32'(32'(32'(aminst_A + aminst_B) + aminst_C[sct_i]) + LR[sct_i]) + lr_next[sct_i]);
                aminst_t_next[sct_i] = 32'(32'(j + i_next[sct_i]) + LC);
    ...
    begin : aminst_mifProc_ff
        if ( ~aminst_rst[sct_i] ) begin
            ...
            LR[sct_i] = 6'd56;
            LA = 6'd57;
            i[sct_i] <= 32'(32'(32'(aminst_A + aminst_B) + aminst_C[sct_i]) + LA);
...
function void topProc_func;
    ...
    sum_next = sum;
    i_2 = s;
    sum_next = sum_next + (signed'({1'b0, 32'(32'(aminst_A + aminst_B) + aminst_C[i_2])}));
    s_next = sum_next;
endfunction
...
begin : topProc_ff
    if ( ~rst ) begin
        ...
        begin
            k = k + (signed'({1'b0, 32'(32'(aminst_A + aminst_B) + aminst_C[i_1])}));
        end
        sum <= k;
    end
    else begin
