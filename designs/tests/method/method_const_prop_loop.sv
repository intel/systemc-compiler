...
begin : loc_var_loop     // test_const_prop_loop.cpp:60:5
    ...
    for (int i = 0; i < s; i++)
    begin
        slot = (slot == 2'd2) ? 1'd0 : 32'(slot + 1'd1);
    end
end
...
function void threadProc_func;
    slot_next = slot0;
    slot_next = (slot_next == 2'd2) ? 1'd0 : 32'(slot_next + 1'd1);
endfunction
...
begin : unstable_loop3a     // test_const_prop_loop.cpp:131:5
    ...
    for (int i = 0; i < 1026; i++)
    begin
        slot = (slot == 1'd1 && !first) ? 1'd0 : 32'(slot + 1'd1);
        first = '0;
    end
end
...
function void simple_for2_func;
    ...
    case (simple_for2_PROC_STATE)
        0: begin
            ...
            simple_for2_PROC_STATE_next = '0; return;    // test_const_prop_loop.cpp:244:13;
        end
        1: begin
            i_next++;
            if (i_next < k_next)
