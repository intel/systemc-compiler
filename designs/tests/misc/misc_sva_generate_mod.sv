...
sctAssertLine98 : assert property (
    @(posedge clk) 9'(m + 1'd1) == 2'd2 || CONST_B |=> 17'(mm - marr[1]) > 2'd3 );
sctAssertLine99 : assert property (
    @(posedge clk) m % 2'd2 != NN || mm == s |=> |marr[2] );
...
sctAssertLine125 : assert property (
    @(posedge clk) |sarr[1 + i] |-> |32'(arr[0] + i) );
...
sctAssertLine133 : assert property (
    @(posedge clk) rec_a |-> |5'(rec_b + 1'd1) );
