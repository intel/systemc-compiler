...
function void test_thread_func;
    ...
    ps_next = s;
    s_next = !(|s);
    if (cntr__next >= 3'd7 || cntr__next == 1'd0)
    begin
        st1_next = 1'd1;
    end else begin
        st1_next = s;
...
function void integer_thread_func;
    ...
    i4_next = i4 - 1'd1;
    i5_next = 6'd42;
    i6_next = signed'({1'b0, i6}) + 2'sd1;
    i7_next = 32'(i7 + 1'd1);
    i8_next = i7 < 5'd20 ? 64'sd12 : 64'(32'(i7 + 1'd1));
    i9_next = 64'(i9 + 2'sd1);
endfunction
...
sctAssertLine54 : assert property (
    @(posedge clk) cntr == 1'd1 |-> $fell(s) );
sctAssertLine55 : assert property (
    @(posedge clk) cntr == 2'd2 |=> $fell(s) );
sctAssertLine56 : assert property (
    @(posedge clk) cntr == 2'd2 |-> $rose(s) );
sctAssertLine58 : assert property (
    @(posedge clk) cntr == 4'd8 |-> $stable(st1) );
sctAssertLine59 : assert property (
    @(posedge clk) cntr == 3'd7 |=> $stable(st1) );
sctAssertLine60 : assert property (
    @(posedge clk) cntr == 4'd8 |-> $stable(st1)[*2] );
sctAssertLine61 : assert property (
    @(posedge clk) cntr == 3'd7 |=> $stable(st1)[*2] );
sctAssertLine63 : assert property (
    @(posedge clk) cntr == 3'd7 |=> $stable(st1)[*4] );
sctAssertLine64 : assert property (
    @(posedge clk) cntr == 4'd15 |=> $stable(st1)[*2] );
sctAssertLine65 : assert property (
    @(posedge clk) cntr == 4'd15 |-> $stable(st1)[*3] );
sctAssertLine67 : assert property (
    @(posedge clk) cntr > 3'd7 && cntr < 4'd15 |-> $stable(st1)[*3] );
...
sctAssertLine114 : assert property (
    @(posedge clk) rstn && i7 < 5'd18 |=> $stable(i8)[*2] );
