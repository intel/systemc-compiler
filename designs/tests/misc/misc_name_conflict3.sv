//==============================================================================
//
// The code is generated by Intel Compiler for SystemC, version 1.6.26
// see more information at https://github.com/intel/systemc-compiler
//
//==============================================================================

//==============================================================================
//
// Module: A ()
//
module A // "a_mod"
(
    input logic clk
);

// Variables generated for SystemC signals
logic rstn;
logic signed [31:0] s;
logic signed [31:0] t1;
logic signed [31:0] t2;

//------------------------------------------------------------------------------
// Method process: local_var (test_name_conflict3.cpp:33:5) 

always_comb 
begin : local_var     // test_name_conflict3.cpp:33:5
    logic a;
    integer i;
    logic [3:0] x;
    x = '0;
    i = s;
    t1 = a ? i : 32'(x);
end

//------------------------------------------------------------------------------
// Clocked THREAD: reg_var1 (test_name_conflict3.cpp:44:5) 

// Thread-local variables
logic signed [31:0] i0;
logic signed [31:0] i_next;
logic signed [31:0] t2_next;

// Next-state combinational logic
always_comb begin : reg_var1_comb     // test_name_conflict3.cpp:44:5
    reg_var1_func;
end
function void reg_var1_func;
    logic signed [63:0] sum;
    i_next = i0;
    t2_next = t2;
    sum = i_next;
    t2_next = sum;
endfunction

// Synchronous register update
always_ff @(posedge clk or negedge rstn) 
begin : reg_var1_ff
    if ( ~rstn ) begin
        i0 <= '0;
    end
    else begin
        i0 <= i_next;
        t2 <= t2_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: reg_var2 (test_name_conflict3.cpp:57:5) 

// Thread-local variables
logic [3:0] x0;
logic [3:0] x_next;
logic signed [63:0] sum0;
logic signed [63:0] sum_next;
logic a0;
logic a_next;
logic signed [31:0] s_next;
logic reg_var2_PROC_STATE;
logic reg_var2_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : reg_var2_comb     // test_name_conflict3.cpp:57:5
    reg_var2_func;
end
function void reg_var2_func;
    a_next = a0;
    s_next = s;
    sum_next = sum0;
    x_next = x0;
    reg_var2_PROC_STATE_next = reg_var2_PROC_STATE;
    
    case (reg_var2_PROC_STATE)
        0: begin
            sum_next = 6'd42 + x_next;
            reg_var2_PROC_STATE_next = 1; return;    // test_name_conflict3.cpp:66:13;
        end
        1: begin
            s_next = sum_next;
            x_next = s;
            sum_next = 6'd42 + x_next;
            reg_var2_PROC_STATE_next = 1; return;    // test_name_conflict3.cpp:66:13;
        end
    endcase
endfunction

// Synchronous register update
always_ff @(posedge clk or negedge rstn) 
begin : reg_var2_ff
    if ( ~rstn ) begin
        a0 <= 1'd1;
        x0 <= '0;
        reg_var2_PROC_STATE <= '0;    // test_name_conflict3.cpp:61:9;
    end
    else begin
        x0 <= x_next;
        sum0 <= sum_next;
        a0 <= a_next;
        s <= s_next;
        reg_var2_PROC_STATE <= reg_var2_PROC_STATE_next;
    end
end

endmodule


