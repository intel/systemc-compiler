...
//------------------------------------------------------------------------------
// Clocked THREAD: init_thread (fifo_shared.h:146:5) 
...
always_comb begin : init_thread_comb     // fifo_shared.h:146:5
    init_thread_func;
end
function void init_thread_func;
    ...
    case (init_thread_PROC_STATE)
        0: begin
            ...
            begin
                init_thread_PROC_STATE_next = 1; return;    // fifo_shared.h:155:40;
            end
            run_req_next = 0;
            run_data_next = 0;
            if (!resp_req || !resp_ready)
            begin
                init_thread_PROC_STATE_next = 2; return;    // fifo_shared.h:162:42;
            end
            data = resp_data;
            resp_ready_next = 0;
            init_thread_PROC_STATE_next = 3; return;    // fifo_shared.h:168:9;
        end
        1: begin
            if (!run_ready || !run_req)
            begin
        ...
        3: begin
            init_thread_PROC_STATE_next = 4; return;    // fifo_shared.h:171:13;
        end
        4: begin
            init_thread_PROC_STATE_next = 4; return;    // fifo_shared.h:171:13;
        end
...
begin : init_thread_ff
    if ( ~nrst ) begin
        ...
        run_data <= 0;
        resp_ready <= 1;
        init_thread_PROC_STATE <= 0;    // fifo_shared.h:151:9;
    end
    else begin
...
//------------------------------------------------------------------------------
// Method process: putMeth (fifo_shared.h:46:5) 

always_comb 
begin : putMeth     // fifo_shared.h:46:5
    logic TMP_0;
    logic TMP_1;
    logic [15:0] data;
    logic TMP_2;
...
//------------------------------------------------------------------------------
// Method process: getMeth (fifo_shared.h:74:5) 

always_comb 
begin : getMeth     // fifo_shared.h:74:5
    logic TMP_0;
    logic [15:0] TMP_1;
    logic [15:0] data;
    resp_req = 0;
