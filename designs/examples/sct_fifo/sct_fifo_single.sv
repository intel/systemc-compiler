...
function void init_thread_func;
    ...
    case (init_thread_PROC_STATE)
        0: begin
            ...
            run_req_next = 1'd1;
            run_data_next = 32'(7'sd32 + i_next);
        ...
        5: begin
            init_thread_PROC_STATE_next = 5; return;    // fifo_single.h:136:13;
        end
    endcase
endfunction
...
//------------------------------------------------------------------------------
// Method process: fifo_asyncProc (sct_fifo.h:328:5) 

always_comb 
begin : fifo_asyncProc     // sct_fifo.h:328:5
    logic outValid;
    logic readyPush;
    logic popIndx;
    logic notEmpty;
    ...
    logic pop;
    popIndx = '0;
    notEmpty = fifo_element_num_d != 1'd0;
    notFull = fifo_element_num_d != 2'd2;
    push = fifo_put_req;
    pop = fifo_get_req;
    outValid = notEmpty;
    readyPush = notFull;
    ...
    if (pop && !push)
    begin
        fifo_element_num = fifo_element_num_d - 1'd1;
    end else begin
        if (!pop && push)
        begin
            fifo_element_num = fifo_element_num_d + 1'd1;
...
//------------------------------------------------------------------------------
// Clocked THREAD: fifo_syncProc (sct_fifo.h:415:5) 
...
// Next-state combinational logic
always_comb begin : fifo_syncProc_comb     // sct_fifo.h:415:5
    fifo_syncProc_func;
end
function void fifo_syncProc_func;
    ...
    if (pop)
    begin
        fifo_pop_indx_next = (fifo_pop_indx == 3'(2'd2 - 1'd1)) ? 1'd0 : fifo_pop_indx + 1'd1;
    end
    if (push)
    begin
        fifo_buffer_next[fifo_push_indx] = fifo_data_in;
        fifo_push_indx_next = (fifo_push_indx == 3'(2'd2 - 1'd1)) ? 1'd0 : fifo_push_indx + 1'd1;
    end
