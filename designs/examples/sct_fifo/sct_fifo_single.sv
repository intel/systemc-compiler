//==============================================================================
//
// The code is generated by Intel Compiler for SystemC, version 1.5.1
...
//------------------------------------------------------------------------------
// Clocked THREAD: init_thread (fifo_single.h:104:5) 
...
always_comb begin : init_thread_comb     // fifo_single.h:104:5
    init_thread_func;
end
function void init_thread_func;
    ...
    case (init_thread_PROC_STATE)
        0: begin
            ...
            if (!run_ready)
            begin
                init_thread_PROC_STATE_next = 1; return;    // fifo_single.h:112:32;
            end
            run_req_next = 1;
            run_data_next = 32 + i_next;
            init_thread_PROC_STATE_next = 2; return;    // fifo_single.h:114:13;
        end
        1: begin
            if (!run_ready)
            begin
        ...
        2: begin
            ...
            if (!resp_req)
            begin
                init_thread_PROC_STATE_next = 3; return;    // fifo_single.h:120:31;
            end
            init_thread_PROC_STATE_next = 4; return;    // fifo_single.h:122:13;
        end
        3: begin
            if (!resp_req)
            begin
        ...
        4: begin
            ...
            init_thread_PROC_STATE_next = 5; return;    // fifo_single.h:125:9;
        end
        5: begin
            init_thread_PROC_STATE_next = 6; return;    // fifo_single.h:129:13;
        end
        6: begin
            init_thread_PROC_STATE_next = 6; return;    // fifo_single.h:129:13;
        end
...
begin : init_thread_ff
    if ( ~nrst ) begin
        ...
        run_data <= 0;
        resp_ready <= 1;
        init_thread_PROC_STATE <= 0;    // fifo_single.h:109:9;
    end
    else begin
...
//------------------------------------------------------------------------------
// Clocked THREAD: threadProc (fifo_single.h:40:5) 
...
// Next-state combinational logic
always_comb begin : threadProc_comb     // fifo_single.h:40:5
    threadProc_func;
end
