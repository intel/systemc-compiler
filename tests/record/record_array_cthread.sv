...
function void rec_loc_arr0_func;
    ...
    ap_b_next = ap_b;
    ap_b_1[1] = 1;
    ap_b_1[0] = signed'({1'b0, ap_b_1[1]}) + ap_b_next;
    i = ap_b_1[0];
endfunction
...
logic signed [1:0] crr_a_next[2][3];
logic [3:0] crr_b[2][3];
...
logic signed [1:0] drr_a_next[2][3];
logic [3:0] drr_b[2][3];
...
logic signed [1:0] err_a_next[2][3][4];
...
logic signed [1:0] frr_a_next[2][3];
logic [3:0] frr_b[2][3];
...
logic signed [1:0] grr_a_next[2][3];
logic [3:0] grr_b[2][3];
...
logic signed [1:0] hrr_a_next[2][3];
logic [3:0] hrr_b[2][3];
...
logic signed [1:0] jrr_a_next[2][3];
logic [3:0] jrr_b[2][3];
...
logic signed [1:0] irr_a_next[2][3];
logic [3:0] irr_b[2][3];
...
function void rec_arr_elem_assign_func;
    logic signed [1:0] lr_a[3];
    logic [3:0] lr_b[3];
    mr_a_next = mr_a;
    mr_b_next = mr_b;
    for (integer i = 0; i < 2; i++)
    begin
        lr_a[i] = mr_a_next[i + 1]; lr_b[i] = mr_b_next[i + 1];
        mr_a_next[i + 1] = mr_a_next[i]; mr_b_next[i + 1] = mr_b_next[i];
    end
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge nrst) 
begin : rec_arr_elem_assign_ff
    if ( ~nrst ) begin
    end
    else begin
        mr_a <= mr_a_next;
...
function void rec_arr_elem_field_assign_func;
    ...
    for (integer i = 0; i < 2; i++)
    begin
        rr_b_next[i] = pr_b_next[i + 1];
        pr_a_next[i + 1] = rr_a_next[i];
    end
endfunction
...
function void rec_arr_elem_const_val2_func;
    ...
    case (rec_arr_elem_const_val2_PROC_STATE)
        ...
        2: begin
            // Call ff1() begin
            i = par_a_next0 + signed'({1'b0, par_b_next0});
            // Call ff1() end
            sr_a_next = 0;
            sr_b_next = 0;
            rec_arr_elem_const_val2_PROC_STATE_next = 1; return;    // test_array_cthread.cpp:517:13;
...
function void rec_arr_elem_func_param_val2_func;
    ...
    tr_a_next = tr_a;
    tr_b_next = tr_b;
    par_a = tr_a_next[1]; par_b = tr_b_next[1];
    // Call f1() begin
    i = par_b;
    // Call f1() end
endfunction
...
function void rec_arr_elem_func_param_val3_func;
    ...
    tr_a_next0 = tr_a0;
    tr_b_next0 = tr_b0;
    par_a = tr_a_next0[i_next6]; par_b = tr_b_next0[i_next6];
    // Call f1() begin
    i = par_b;
    // Call f1() end
endfunction
...
logic signed [1:0] wrr_a_next[2];
logic [3:0] wrr_b[2];
logic [3:0] wrr_b_next[2];
logic signed [1:0] wr_a[3];
logic signed [1:0] wr_a_next[3];
logic [3:0] wr_b[3];
...
function void rec_arr_elem_func_param_ref2_func;
    ...
    case (rec_arr_elem_func_param_ref2_PROC_STATE)
        0: begin
            // Call f2_two() begin
            k = wr_b_next[1] + wrr_b_next[i_next7];
            // Call f2_two() end
            // Call f2_two() begin
...
function void rec_arr_elem_func_param_cref2_func;
    ...
    case (rec_arr_elem_func_param_cref2_PROC_STATE)
        ...
        1: begin
            // Call cref_wait() begin
            res_next = res_next + signed'({1'b0, cvrr_b_next[2]});
            TMP_0 = res_next;
            // Call cref_wait() end
            res_1 = TMP_0;
            rec_arr_elem_func_param_cref2_PROC_STATE_next = 2; return;    // test_array_cthread.cpp:673:13;
