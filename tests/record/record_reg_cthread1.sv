//==============================================================================
//
// The code is generated by Intel Compiler for SystemC, version 1.3.7
// see more information at https://github.com/intel/systemc-compiler
//
//==============================================================================

//==============================================================================
//
// Module: A ()
//
module A // "a"
(
    input logic clk
);

// SystemC signals
logic rstn;

//------------------------------------------------------------------------------
// Clocked THREAD: record_assign (test_reg_cthread1.cpp:46:5) 

// Thread-local variables
logic signed [31:0] s_sin;
logic signed [31:0] s_sin_next;
logic signed [31:0] s_cos;
logic signed [31:0] s_cos_next;
logic signed [31:0] r_sin;
logic signed [31:0] r_sin_next;
logic signed [31:0] r_cos;
logic signed [31:0] r_cos_next;
logic record_assign_PROC_STATE;
logic record_assign_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : record_assign_comb     // test_reg_cthread1.cpp:46:5
    record_assign_func;
end
function void record_assign_func;
    integer b;
    r_cos_next = r_cos;
    r_sin_next = r_sin;
    s_cos_next = s_cos;
    s_sin_next = s_sin;
    record_assign_PROC_STATE_next = record_assign_PROC_STATE;
    
    case (record_assign_PROC_STATE)
        0: begin
            s_sin_next = 1;
            r_sin_next = s_sin_next; r_cos_next = s_cos_next;
            record_assign_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:53:13;
        end
        1: begin
            b = r_sin_next;
            s_sin_next = 1;
            r_sin_next = s_sin_next; r_cos_next = s_cos_next;
            record_assign_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:53:13;
        end
    endcase
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_assign_ff
    if ( ~rstn ) begin
        s_sin <= 1;
        s_cos <= 2;
        record_assign_PROC_STATE <= 0;    // test_reg_cthread1.cpp:48:9;
    end
    else begin
        s_sin <= s_sin_next;
        s_cos <= s_cos_next;
        r_sin <= r_sin_next;
        r_cos <= r_cos_next;
        record_assign_PROC_STATE <= record_assign_PROC_STATE_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_usedef_assign (test_reg_cthread1.cpp:59:5) 

// Thread-local variables
logic signed [31:0] r_sin0;
logic signed [31:0] r_sin_next0;
logic signed [31:0] r_cos0;
logic signed [31:0] r_cos_next0;
logic record_usedef_assign_PROC_STATE;
logic record_usedef_assign_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : record_usedef_assign_comb     // test_reg_cthread1.cpp:59:5
    record_usedef_assign_func;
end
function void record_usedef_assign_func;
    integer i;
    r_cos_next0 = r_cos0;
    r_sin_next0 = r_sin0;
    record_usedef_assign_PROC_STATE_next = record_usedef_assign_PROC_STATE;
    
    case (record_usedef_assign_PROC_STATE)
        0: begin
            r_sin_next0 = 1;
            r_cos_next0 = 2;
            record_usedef_assign_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:64:13;
        end
        1: begin
            i = r_sin_next0;
            r_sin_next0 = 1;
            r_cos_next0 = 2;
            record_usedef_assign_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:64:13;
        end
    endcase
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_usedef_assign_ff
    if ( ~rstn ) begin
        record_usedef_assign_PROC_STATE <= 0;    // test_reg_cthread1.cpp:60:9;
    end
    else begin
        r_sin0 <= r_sin_next0;
        r_cos0 <= r_cos_next0;
        record_usedef_assign_PROC_STATE <= record_usedef_assign_PROC_STATE_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_glob_reg (test_reg_cthread1.cpp:72:5) 

// Thread-local variables
logic signed [31:0] gr_sin;
logic signed [31:0] gr_sin_next;
logic signed [31:0] gr_cos;
logic signed [31:0] gr_cos_next;
logic record_glob_reg_PROC_STATE;
logic record_glob_reg_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : record_glob_reg_comb     // test_reg_cthread1.cpp:72:5
    record_glob_reg_func;
end
function void record_glob_reg_func;
    integer b;
    gr_cos_next = gr_cos;
    gr_sin_next = gr_sin;
    record_glob_reg_PROC_STATE_next = record_glob_reg_PROC_STATE;
    
    case (record_glob_reg_PROC_STATE)
        0: begin
            gr_sin_next = 1;
            record_glob_reg_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:77:13;
        end
        1: begin
            b = gr_sin_next;
            gr_sin_next = 1;
            record_glob_reg_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:77:13;
        end
    endcase
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_glob_reg_ff
    if ( ~rstn ) begin
        record_glob_reg_PROC_STATE <= 0;    // test_reg_cthread1.cpp:73:9;
    end
    else begin
        gr_sin <= gr_sin_next;
        gr_cos <= gr_cos_next;
        record_glob_reg_PROC_STATE <= record_glob_reg_PROC_STATE_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_glob_assign1 (test_reg_cthread1.cpp:84:5) 

// Thread-local variables
logic signed [31:0] grr_sin;
logic signed [31:0] grr_sin_next;
logic signed [31:0] grr_cos;
logic signed [31:0] grr_cos_next;

// Next-state combinational logic
always_comb begin : record_glob_assign1_comb     // test_reg_cthread1.cpp:84:5
    record_glob_assign1_func;
end
function void record_glob_assign1_func;
    integer r_sin;
    integer r_cos;
    grr_cos_next = grr_cos;
    grr_sin_next = grr_sin;
    grr_cos_next = 1;
    r_sin = grr_sin_next; r_cos = grr_cos_next;
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_glob_assign1_ff
    if ( ~rstn ) begin
    end
    else begin
        grr_sin <= grr_sin_next;
        grr_cos <= grr_cos_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_glob_assign1a (test_reg_cthread1.cpp:97:5) 

// Thread-local variables
logic signed [31:0] gp_sin;
logic signed [31:0] gp_sin_next;
logic signed [31:0] gp_cos;
logic signed [31:0] gp_cos_next;

// Next-state combinational logic
always_comb begin : record_glob_assign1a_comb     // test_reg_cthread1.cpp:97:5
    record_glob_assign1a_func;
end
function void record_glob_assign1a_func;
    integer r_sin;
    integer r_cos;
    gp_cos_next = gp_cos;
    gp_sin_next = gp_sin;
    r_sin = 1;
    r_cos = 2;
    r_sin = gp_sin_next; r_cos = gp_cos_next;
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_glob_assign1a_ff
    if ( ~rstn ) begin
    end
    else begin
        gp_sin <= gp_sin_next;
        gp_cos <= gp_cos_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_glob_assign2 (test_reg_cthread1.cpp:109:5) 

// Thread-local variables
logic signed [31:0] r_sin1;
logic signed [31:0] r_sin_next1;
logic signed [31:0] r_cos1;
logic signed [31:0] r_cos_next1;
logic signed [31:0] gpp_sin;
logic signed [31:0] gpp_cos;
logic record_glob_assign2_PROC_STATE;
logic record_glob_assign2_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : record_glob_assign2_comb     // test_reg_cthread1.cpp:109:5
    record_glob_assign2_func;
end
function void record_glob_assign2_func;
    r_cos_next1 = r_cos1;
    r_sin_next1 = r_sin1;
    record_glob_assign2_PROC_STATE_next = record_glob_assign2_PROC_STATE;
    
    case (record_glob_assign2_PROC_STATE)
        0: begin
            r_sin_next1 = 1;
            r_cos_next1 = 2;
            record_glob_assign2_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:114:13;
        end
        1: begin
            gpp_sin = r_sin_next1; gpp_cos = r_cos_next1;
            r_sin_next1 = 1;
            r_cos_next1 = 2;
            record_glob_assign2_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:114:13;
        end
    endcase
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_glob_assign2_ff
    if ( ~rstn ) begin
        record_glob_assign2_PROC_STATE <= 0;    // test_reg_cthread1.cpp:110:9;
    end
    else begin
        r_sin1 <= r_sin_next1;
        r_cos1 <= r_cos_next1;
        record_glob_assign2_PROC_STATE <= record_glob_assign2_PROC_STATE_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_not_defined (test_reg_cthread1.cpp:121:5) 

// Thread-local variables
logic signed [31:0] rn_sin;
logic signed [31:0] rn_sin_next;
logic signed [31:0] rn_cos;
logic signed [31:0] rn_cos_next;
logic record_not_defined_PROC_STATE;
logic record_not_defined_PROC_STATE_next;

// Next-state combinational logic
always_comb begin : record_not_defined_comb     // test_reg_cthread1.cpp:121:5
    record_not_defined_func;
end
function void record_not_defined_func;
    integer i;
    rn_cos_next = rn_cos;
    rn_sin_next = rn_sin;
    record_not_defined_PROC_STATE_next = record_not_defined_PROC_STATE;
    
    case (record_not_defined_PROC_STATE)
        0: begin
            rn_sin_next = 1;
            rn_cos_next = 2;
            record_not_defined_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:126:13;
        end
        1: begin
            i = rn_sin_next;
            rn_sin_next = 1;
            rn_cos_next = 2;
            record_not_defined_PROC_STATE_next = 1; return;    // test_reg_cthread1.cpp:126:13;
        end
    endcase
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_not_defined_ff
    if ( ~rstn ) begin
        record_not_defined_PROC_STATE <= 0;    // test_reg_cthread1.cpp:122:9;
    end
    else begin
        rn_sin <= rn_sin_next;
        rn_cos <= rn_cos_next;
        record_not_defined_PROC_STATE <= record_not_defined_PROC_STATE_next;
    end
end

//------------------------------------------------------------------------------
// Clocked THREAD: record_glob_not_defined (test_reg_cthread1.cpp:133:5) 

// Thread-local variables
logic signed [31:0] gn_sin;
logic signed [31:0] gn_sin_next;
logic signed [31:0] gn_cos;
logic signed [31:0] gn_cos_next;

// Next-state combinational logic
always_comb begin : record_glob_not_defined_comb     // test_reg_cthread1.cpp:133:5
    record_glob_not_defined_func;
end
function void record_glob_not_defined_func;
    integer r_sin;
    integer r_cos;
    gn_cos_next = gn_cos;
    gn_sin_next = gn_sin;
    r_sin = gn_sin_next; r_cos = gn_cos_next;
endfunction

// Syncrhonous register update
always_ff @(posedge clk or negedge rstn) 
begin : record_glob_not_defined_ff
    if ( ~rstn ) begin
    end
    else begin
        gn_sin <= gn_sin_next;
        gn_cos <= gn_cos_next;
    end
end

endmodule


