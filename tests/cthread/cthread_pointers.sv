...
// C++ data variables
localparam logic [31:0] N = 'd4;
localparam logic [31:0] M = 'd5;
localparam logic [31:0] K = 'd5;
...
function void array_ptr1_func;
    ...
    begin
        ...
        begin
            mul = mul * (parr2[i][j] + iarr1[i]);
            dif = dif - signed'({1'b0, parr2[i][j]});
        end
        parr1_next[i] = signed'({1'b0, mul}) * dif;
    end
endfunction
...
function void array_ptr2_func;
    ...
    for (integer j = 0; j < M; j++)
    begin
        parr2_next[i_1][j] = parr3[i_1][j][i_1 + 1];
    end
    j_1 = |s ? i_1 : 1;
    parr3_next[3][i_1][1] = 32'(s);
    parr2_next[i_1][j_1] = 64'(s);
endfunction
