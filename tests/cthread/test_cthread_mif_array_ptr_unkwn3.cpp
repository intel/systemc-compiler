...
// Modular interface array [2][1], contains signal/signal pointer arrays
struct mod_if : public sc_module, sc_interface 
{
    ...
    void thread_member_reg() 
    {
        ...
        *asp[j] = 0;
        ...
        while (true) {
            int l = w + aw[j] + as[j+1].read() + asp[j+2]->read();
            wait();
        }
    }
};
...
SC_MODULE(Top) {
    ...
    void top_thread_comb() 
    {
        ...
        while (true) {
            minst[1][0]->y = minst[j][0]->asp[0]->read();
            int i = minst[1][0]->y; 
            
            minst[0][0]->ay[1] = minst[j][j+1]->as[j].read();
            i = minst[0][0]->ay[1];
