...
public:
    ...
    void sct_assert_several()
    {
        ...
        SCT_ASSERT(b || s, SCT_TIME(2,3), s_d2.read() && ((*ps).read() == 1));
        SCT_ASSERT(psarr[1]->read(), SCT_TIME(M, M+N), sarr[0].read() != arr[2]);
        wait();
